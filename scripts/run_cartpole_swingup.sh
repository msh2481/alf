...
BASE_DIR="/tmp/cartpole_swingup/${NAME}"

if [ -n "$SEEDS" ] && [ "$SEEDS" -gt 0 ] 2>/dev/null; then
    echo "Running multi-seed batch: num_seeds=$SEEDS num_agents=$NUM_AGENTS lr=$LR wd=$WD prior_scale=$PRIOR_SCALE ln=$LN utd=$UTD tau=$TAU async=$ASYNC env=$ENV base_dir=$BASE_DIR"
    ...
    ROOT_DIR="$BASE_DIR"
    echo "Running single run: num_agents=$NUM_AGENTS lr=$LR wd=$WD prior_scale=$PRIOR_SCALE ln=$LN utd=$UTD tau=$TAU async=$ASYNC env=$ENV root_dir=$ROOT_DIR"
