...
def _define_flags():
    ...
    flags.DEFINE_string(
        "common_gin",
        "",
        "Common config for the two sides, "
...
def _return_diff(item):
    return abs(item[AVG_R_DIFF]) / (
        max(
            ...
            abs(float(item["alg2_" + AVG_R_METRIC])),
        )
        + 1.0e-5
    )
...
def _play_cmd(root_dir, seed):
    return (
        ...
        " --epsilon_greedy=0 {g}"
    ).format(root_dir=root_dir, seed=seed, g=FLAGS.common_gin)
    ...
    <script>
        $(document).ready(function () {
            $('#table_id').DataTable({
                ...
                "order": [["""
        + '{}, "desc"], [{}, "asc"]]'.format(avgreturn_index, seed_index)
        + r"""
            });
        });
    </script>
