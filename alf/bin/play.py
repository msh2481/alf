...
def _define_flags():
    ...
    flags.DEFINE_float(
        "sleep_time_per_step", 0.01, "sleep so many seconds for each step"
    )
    flags.DEFINE_string(
        "record_file",
        None,
        "If provided, video will be recorded"
        "to a file instead of shown on the screen.",
    )
    # use '--norender' to disable frame rendering
    flags.DEFINE_bool(
        "render", True, "Whether render ('human'|'rgb_array') the frames or not"
    )
    # use '--alg_render' to enable algorithm specific rendering
    flags.DEFINE_bool(
        "alg_render", False, "Whether enable algorithm specific rendering"
    )
    ...
    flags.DEFINE_bool(
        "selective_mode",
        False,
        "Whether use the selective mode. "
...
def play():
    ...
    if FLAGS.selective_mode:
        assert FLAGS.record_file is not None, (
            "Should provide a valid value " "for `record_file`"
        )
    ...
    try:
        policy_trainer.play(
            ...
            ignored_parameter_prefixes=(
                FLAGS.ignored_parameter_prefixes.split(",")
                if FLAGS.ignored_parameter_prefixes
                else []
            ),
        )
...
def launch_snapshot_play():
    ...
    try:
        subprocess.check_call(
            args, env=env_vars, stdout=sys.stdout, stderr=sys.stdout, shell=False
        )
