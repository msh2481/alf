...
class MyAlg(Algorithm):
    ...
    def __init__(
        self, optimizer=None, sub_algs=[], params=[], checkpoint=None, name="MyAlg"
    ):
...
class AlgorithmTest(alf.test.TestCase):
    ...
    def test_get_optimizer_info(self):
        ...
        alg_root = MyAlg(
            optimizer=alf.optimizers.Adam(lr=0.25), sub_algs=[alg_1], name="root"
        )
        ...
        alg_root = MyAlg(
            optimizer=alf.optimizers.Adam(lr=0.25), sub_algs=[alg_1, alg_1], name="root"
        )
        info = json.loads(alg_root.get_optimizer_info())
        self.assertEqual(len(info), 1)
        self.assertEqual(info[0]["parameters"], [alg_root.get_param_name(param_1)])
        ...
        alg_root = MyAlg(
            optimizer=alf.optimizers.Adam(lr=0.25), sub_algs=[alg_1, alg_2], name="root"
        )
    ...
    def test_get_optimizer_info2(self):
        ...
        alg_2.layer = layer
        alg_root = MyAlg(
            sub_algs=[alg_1, alg_2], optimizer=alf.optimizers.Adam(lr=0.25), name="root"
        )
        info = json.loads(alg_root.get_optimizer_info())
        self.assertEqual(
    ...
    def test_partial_preloading_and_then_checkpoint_loading(self):
        ...
        with tempfile.TemporaryDirectory() as ckpt_dir:
            ...
            # checkpoint
            new_alg_1 = MyAlg(
                params=[nn.Parameter(torch.tensor([-200.0]))], checkpoint=ckpt_path
