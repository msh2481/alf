...
AgentState = namedtuple(
    "AgentState", ["rl", "irm", "goal_generator", "repr", "rw"], default_value=()
)
...
class Agent(RLAlgorithm):
    ...
    def __init__(
        ...
        if representation_learner_cls is not None:
            ...
            assert hasattr(
                representation_learner, "output_spec"
        ...
        if goal_generator is not None:
            ...
            rl_observation_spec = [rl_observation_spec, goal_generator.action_spec]
        ...
        if isinstance(rl_algorithm, LatentMbrlAlgorithm):
            assert isinstance(
                representation_learner, PredictiveRepresentationLearner
            ), ("need to use " "PredictiveRepresentationLearner")
    ...
    def predict_step(self, time_step: TimeStep, state: AgentState):
        ...
        if self._goal_generator is not None:
            goal_step = self._goal_generator.predict_step(
                time_step._replace(observation=observation), state.goal_generator
            )
    ...
    def train_step(self, time_step: TimeStep, state, rollout_info):
        ...
        rl_step = self._rl_algorithm.train_step(
            time_step._replace(observation=observation), state.rl, rollout_info.rl
        )
    ...
    def calc_loss_offline(self, info, pre_train):
        ...
        return self._agent_helper.accumulate_loss_info(
            algorithms, info, True, pre_train
        )
    ...
    def summarize_rollout(self, experience):
        ...
        if (
            hasattr(self._rl_algorithm, "summarize_rollout")
