...
class RLAlgorithm(Algorithm):
    ...
    def __init__(
        ...
        if reward_spec.numel > 1:
            if reward_weights:
                ...
                self.register_buffer(
                    "_reward_weights", torch.tensor(reward_weights, dtype=torch.float32)
                )
            else:
                self.register_buffer(
                    "_reward_weights",
        ...
        if self._env is not None and not self.on_policy:
            ...
            self.set_replay_buffer(
                self._env.batch_size, replay_buffer_length, config.priority_replay
            )
        ...
        if env is not None:
            ...
            self._metrics = [
                ...
                alf.metrics.AverageReturnMetric(
                    buffer_size=metric_buf_size, example_time_step=example_time_step
                ),
                alf.metrics.AverageEpisodeLengthMetric(
                    example_time_step=example_time_step, buffer_size=metric_buf_size
                ),
    ...
    def _async_unroll(self, unroll_length: int):
        ...
        alf.summary.scalar(
            "time/unroll_env_step", env_step_time, average_over_summary_interval=True
        )
    ...
    def _unroll_iter_off_policy(self):
        ...
        if (
            ...
            with torch.set_grad_enabled(
                config.unroll_with_grad
            ), torch.cuda.amp.autocast(config.enable_amp, dtype=self._config.amp_dtype):
                with record_time("time/unroll"):
                    self.eval()
