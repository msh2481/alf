...
class LagrangianRewardWeightAlgorithmTest(parameterized.TestCase, alf.test.TestCase):
    ...
    def test_lagrangian_algorithm(self, reward_weight_normalization):
        ...
        if not reward_weight_normalization:
            ...
            self.assertEqual(float(reward_weights[0]), 1.0)
            self.assertEqual(float(reward_weights[3]), 1.0)
            # weight increased (0 < 0.1)
            self.assertGreater(float(reward_weights[1]), 1.0)
            # weight decreased (0 > -0.1)
            self.assertGreater(1.0, float(reward_weights[2]))
        else:
            self.assertTensorClose(reward_weights.sum(), torch.tensor(1.0))
