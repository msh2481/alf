...
class MCTSModel(nn.Module, metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        if reset_reward_sum_period > 0:
            assert predict_reward_sum, (
                "reset_reward_sum_preiod can only be "
        ...
        if self._train_repr_prediction:
            with alf.summary.scope(self._name):
                ...
                repr_loss = self._repr_loss(
                    model_output.state.state, target.observation
                )
            ...
            with alf.summary.scope(self._name):
                alf.summary.scalar(
                    ...
                    tensor_utils.explained_variance(
                        model_output.value[:, 0], target.value[:, 0]
                    ),
                )
                alf.summary.scalar(
                    "explained_variance_of_value1",
                    tensor_utils.explained_variance(
                        model_output.value[:, 1:], target.value[:, 1:], dim=0
                    ).mean(),
                )
                if self._train_reward_function:
                    alf.summary.scalar(
...
class SimpleMCTSModel(MCTSModel):
    ...
    def prediction_model(self, dyn_state, pred_state):
        (value_pred, reward_pred, action_distribution, game_over_logit), pred_state = (
            self._prediction_net(dyn_state, pred_state)
        )
