...
TimestepItem = namedtuple(
    "TimestepItem", ["step_type", "observation", "reward", "env_id"], default_value=()
)
...
class RewardTransformerTest(parameterized.TestCase, alf.test.TestCase):
    ...
    @parameterized.parameters(
        ...
        alf.algorithms.data_transformer.RewardNormalizer(update_mode="rollout"),
    )
...
class FunctionalRewardTransformerTest(parameterized.TestCase, alf.test.TestCase):
    ...
    def test_multi_dim_reward_transformation(
        self, reward_dim, dim_for_trans, scale, bias
    ):
...
class FrameStackerTest(parameterized.TestCase, alf.test.TestCase):
    ...
    def test_frame_stacker(self, stack_axis=0):
        ...
        state = common.zero_tensor_from_nested_spec(frame_stacker.state_spec, 2)
        ...
        for t in range(1000):
            time_step = TimestepItem(
                step_type=torch.tensor([_step_type(t, 17), _step_type(t, 22)]),
                observation=alf.nest.map_structure(lambda x: x[t], observation),
            )
            batch = Experience(time_step=time_step)
            replay_buffer.add_batch(batch)
            timestep, state = frame_stacker.transform_timestep(time_step, state)
            if t == 0:
                for b in (0, 1):
...
class ImageScaleTransformerTest(alf.test.TestCase):
    ...
    def test_image_scale_transformer(self):
        spec = alf.BoundedTensorSpec(
            (3, 16, 16), dtype=torch.uint8, minimum=0, maximum=255
        )
        transformer = ImageScaleTransformer(spec, min=0.0)
        new_spec = transformer.transformed_observation_spec
        self.assertEqual(new_spec.dtype, torch.float32)
        self.assertEqual(new_spec.minimum, 0.0)
        self.assertEqual(new_spec.maximum, 1.0)
        ...
        self.assertLess(
            (transformed.observation * 255 - timestep.observation).abs().max(), 1e-4
        )
        experience = Experience(time_step=timestep)
        transformed_exp = transformer.transform_experience(experience)
        self.assertLess(
            (transformed_exp.observation * 255 - timestep.observation).abs().max(), 1e-4
        )

        spec = dict(
            img=alf.BoundedTensorSpec(
                (3, 16, 16), dtype=torch.uint8, minimum=0, maximum=255
            ),
            other=alf.TensorSpec(()),
        )
        self.assertRaises(AssertionError, ImageScaleTransformer, spec, min=0.0)
        self.assertRaises(
            AssertionError, ImageScaleTransformer, spec, min=0.0, fields=["other"]
        )
        transformer = ImageScaleTransformer(spec, min=0.0, fields=["img"])
...
class HindsightExperienceTransformerTest(ReplayBufferTest):
    ...
    def test_compute_her_future_step_distance(self, end_prob):
        ...
        steps = self.generate_step_types(num_envs, max_steps, end_prob=end_prob)
        for t in range(max_steps):
            for b in range(num_envs):
                batch = get_exp_batch(
                    [b], self.dim, t=steps[b * max_steps + t], x=1.0 / max_steps * t + b
                )
            ...
            if t > 1:
                ...
                self.assertEqual(
                    list(res.get_time_step_field("o.g").shape), [sample_steps, 2]
                )
