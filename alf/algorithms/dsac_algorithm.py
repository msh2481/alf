...
class DSacAlgorithm(SacAlgorithm):
    ...
    def __init__(
        ...
        """
        assert tau_type in ("fixed", "iqn"), f"Unsupported tau_type: {tau_type}."
    ...
    def _compute_critics(
        ...
        critic_quantiles, critics_state = critic_net(
            (critic_inputs, tau_hat), state=critics_state
        )
        extra_info = ()
    ...
    def _get_actor_q_value(self, inputs: TimeStep, state, action, tau_info: TauInfo):
        ...
        critics, critics_state, critics_std = self._compute_critics(
            self._critic_networks, inputs.observation, action, tau_hat, state, delta_tau
        )
    ...
    def _actor_train_step(
        self, inputs: TimeStep, state, action, log_pi, tau_info: TauInfo
    ):
