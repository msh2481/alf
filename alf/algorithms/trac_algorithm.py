...
TracInfo = namedtuple(
    "TracInfo", ["action_distribution", "observation", "state", "ac", "prev_action"]
)
...
class TracAlgorithm(RLAlgorithm):
    ...
    def calc_loss(self, info: TracInfo):
        ...
        ac_info = info.ac._replace(action_distribution=info.action_distribution)
    ...
    def _calc_change(self, exp_array):
        ...
        def _dist(d1, d2):
            ...
            if isinstance(d1_base, td.Categorical):
                ...
                dist = td.kl.kl_divergence(d1, d2) + td.kl.kl_divergence(d2, d1)
        ...
        total_dists = nest_map(lambda _: torch.tensor(0.0), self.action_spec)
