...
VqvaeLossInfo = namedtuple(
    "VqvaeLossInfo", ["quantization", "commitment", "reconstruction"], default_value=()
)
...
class Vqvae(Algorithm):
    ...
    def __init__(
        ...
        torch.nn.init.uniform_(
            self._embedding, a=-1 / self._num_embeddings, b=1 / self._num_embeddings
        )
        ...
        if optimizer is not None:
            self.add_optimizer(
                optimizer, [self._encoding_net, self._decoding_net, self._embedding]
            )
