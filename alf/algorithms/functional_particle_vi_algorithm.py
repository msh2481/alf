...
class FuncParVIAlgorithm(ParVIAlgorithm):
    ...
    * Outputs of the neural network for an input mini-batch as a particle.
    ...
    def train_step(
        self, inputs, entropy_regularization=None, loss_mask=None, state=None
    ):
    ...
    def _function_neglogprob(self, targets, outputs):
        ...
        if self._loss_type == "regression":
            # [B, D] -> [B, N, D]
            targets = _expand_to_replica(
                targets, num_particles, self._param_net.output_spec
            )
            # [B, N, D] -> [N, B, D]
    ...
    def _neglogprob(self, inputs, params):
        ...
        if self._loss_type == "regression":
            # [B, d] -> [B, N, d]
            target = _expand_to_replica(
                target, num_particles, self._param_net.output_spec
            )
        else:
            # [B] -> [B, N]
