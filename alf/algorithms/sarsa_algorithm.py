...
class SarsaAlgorithm(RLAlgorithm):
    ...
    def _get_action(
        self, actor_network, time_step: TimeStep, state: SarsaState, epsilon_greedy=1.0
    ):
        ...
        if actor_network.is_distribution_output:
            ...
            def _sample(a, noise):
                if epsilon_greedy >= 1.0:
                    ...
                    return torch.where(
                        common.expand_dims_as(choose_random_action, a), a + noise, a
                    )
    ...
    def calc_loss(self, info: SarsaInfo):
        ...
        step_type0 = torch.where(
            step_type0 == StepType.FIRST, torch.tensor(StepType.LAST), step_type0
        )
