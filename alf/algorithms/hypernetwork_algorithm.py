    ...
    * The generator may be trained with generative particle-based variational
    ...
    def __init__(
        ...
        if function_vi:
            ...
            assert function_extra_bs_sampler in (
                "uniform",
                "normal",
        ...
        self._config = config
        assert voting in ["soft", "hard"], 'voting only supports "soft" and "hard"'
        self._voting = voting
    ...
    def set_data_loader(
        ...
        if entropy_regularization is None:
            self._entropy_regularization = (
                train_loader.batch_size / train_loader.dataset.data.shape[0]
            )

        if outlier_data_loaders is not None:
            assert isinstance(outlier_data_loaders, tuple), (
                "outlier dataset "
        ...
        with record_time("time/train"):
            ...
            for batch_idx, (data, target) in enumerate(self._train_loader):
                ...
                alg_step = self.train_step(
                    (data, target), num_particles=num_particles, state=state
                )
    ...
    def train_step(
        self, inputs, num_particles=None, entropy_regularization=None, state=None
    ):
    ...
    def _function_transform(self, data, params):
        ...
        if isinstance(params, tuple):
            ...
            if self._function_extra_bs_sampler == "uniform":
                noise.uniform_(0.0, 1.0)
            else:
                noise.normal_(mean=0.0, std=self._function_extra_bs_std)
    ...
    def eval_uncertainty(self, num_particles=None):
        ...
        * AUROC (AUC): AUC is computed with respect to the entropy in the
        ...
        with torch.no_grad():
            ...
            outputs_outlier = predict_dataset(
                self._param_net, self._outlier_test_loader
            )
    ...
    def summarize_train(
        self, loss_info, params, cum_loss=None, avg_acc=None, inverse_mvp_loss=None
    ):
