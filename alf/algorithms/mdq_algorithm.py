...
class MdqAlgorithm(OffPolicyAlgorithm):
    ...
    def _predict(self, time_step: TimeStep, state=None, epsilon_greedy=1.0):
    ...
    def _critic_train_step(
        ...
        critic, critic_state = self._critic_networks(
            torch.cat(critic_input, -1), alpha=alpha, state=state.critic, free_form=True
        )

        noisy_distill_action = self._get_noisy_action(
            action, self._action_spec, self._distill_noise, noise_clip=0, spec_clip=True
        )

        critic_distill_input = (inputs.observation, noisy_distill_action.detach())

        # [B, n, action_dim]
        critic_adv_form, critic_state = self._critic_networks(
            critic_distill_input, alpha=alpha, state=state.critic, free_form=False
        )

        target_critic_input_new = (
            tensor_utils.tensor_extend_new_dim(
                target_critic_input[0], dim=1, n=self._num_critic_replicas
            ),
            target_critic_input[1],
        )

        distill_critic_input_new = (
            tensor_utils.tensor_extend_new_dim(
        ...
        kl_wrt_prior = tensor_utils.reverse_cumsum(kl_wrt_prior_per_dim, dim=-1)
        info = MdqCriticInfo(
    ...
    def calc_loss(self, info: MdqInfo):
        ...
        return LossInfo(
            loss=total_loss,
            extra=MdqLossInfo(
                critic=critic_loss.extra, alpha=alpha_loss.extra, distill=distill_loss
            ),
        )
    ...
    def _calc_critic_loss(self, train_info: MdqInfo):
        ...
        for j in range(num_critic_replicas):
            critic_losses.append(
                self._critic_losses[j](
                    info=train_info,
                    value=critic_free_form[:, :, j],
                    target_value=target_critic_corrected,
                ).loss
            )
        ...
        distill_loss = (
            critic_adv_form[..., -1] - distill_target.unsqueeze(2).detach()
        ) ** 2
        # mean over replica
        distill_loss = distill_loss.mean(dim=2)

        return (
            LossInfo(loss=critic_loss, extra=critic_loss / len(critic_losses)),
            distill_loss,
        )

    def _get_noisy_action(
        self, actions, action_specs, noise_level, noise_clip=0.0, spec_clip=True
    ):
        if noise_level > 0:
