...
class IqlAlgorithm(OffPolicyAlgorithm):
    ...
    def _actor_train_step(
        self, inputs: TimeStep, state, action_distribution, v_value, rollout_info
    ):

        # IQL uses target critic network for computing the value learning target
        q_value, critics_state = self._compute_critics(
            self._target_critic_networks, inputs.observation, rollout_info.action, state
        )
    ...
    def train_step(self, inputs: TimeStep, state: IqlState, rollout_info: IqlInfo):
        ...
        actor_state, actor_loss = self._actor_train_step(
            inputs, state.actor, action_distribution, critic_info.value, rollout_info
        )
