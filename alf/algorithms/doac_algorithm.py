...
class DOacAlgorithm(DSacAlgorithm):
    ...
    def __init__(
        ...
        self._explore_tau_hat, _ = self._get_tau(batch_size=1, tau_type="fixed")
    ...
    def _predict_action(
        ...
        if rollout and not self._training_started:
            ...
            action_dist_type = nest.map_structure(
                lambda dist: isinstance(dist, td.TransformedDistribution), action_dist
            )
            assert all(
            ...
            action_dist = nest.map_structure(
                lambda base_dist, transform_dist: td.TransformedDistribution(
                    base_distribution=base_dist, transforms=transform_dist.transforms
                ),
                normal_dist,
                action_dist,
            )
