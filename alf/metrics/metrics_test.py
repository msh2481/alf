...
def _create_timestep(reward, env_id, step_type, env_info):
    return TimeStep(
        ...
        discount=torch.where(
            to_tensor(step_type) == StepType.LAST, torch.tensor(0.0), torch.tensor(1.0)
        ),
...
class AverageDrivingMetric(AverageEpisodicAggregationMetric):
    ...
    def _extract_metric_values(self, time_step):
        return {
            "velocity@step": time_step.env_info["kinetics"]["velocity"],
            "acceleration@step": time_step.env_info["kinetics"]["acceleration"],
            "success": time_step.env_info["success"],
        }
...
class THMetricsTest(parameterized.TestCase, unittest.TestCase):
    ...
    def _create_trajectories(self, vector_reward=False):
        ...
        ts4 = timestep_mid(
            _vectorize_reward([5, 6]),
            [1, 2],
            dict(x=to_tensor([2, -2]), y=to_tensor([-1, -6])),
        )
        ts5 = timestep_last(
            _vectorize_reward([7, 8]),
            [1, 2],
    ...
    @parameterized.named_parameters(
        [
            ...
            ("testAverageEpisodeLength", AverageEpisodeLengthMetric, 6, 2.0, False),
            (
                "testAverageEnvInfoMetric",
                AverageEnvInfoMetric,
                6,
                dict(x=torch.as_tensor(5.0), y=torch.as_tensor(1.5)),
                False,
            ),
            (
                "testAverageDiscountedReturnMetric",
                AverageDiscountedReturnMetric,
                6,
                7.2225,
                False,
            ...
            ("testAverageReturnVectorReward", AverageReturnMetric, 6, [9.0, 9.0], True),
            (
    ...
    def testMetric(
        self, metric_class, num_trajectories, expected_result, vector_reward
    ):
        ...
        if metric_class == AverageEnvInfoMetric:
            self.assertEqual(
                dict(x=torch.as_tensor(0.0), y=torch.as_tensor(0.0)), metric.result()
            )
        else:
    ...
    def test_accumulator_mask(self):
        ...
        traj.append(
            timestep_mid(
                ...
                env_id=[1, 2],
                env_info={
                    "velocity@max": to_tensor([neg_inf, -1.0]),
        ...
        traj.append(
            timestep_last(
                ...
                env_id=[1, 2],
                env_info={
                    "velocity@max": to_tensor([neg_inf, -2.0]),
                    "success": to_tensor([0.0, 0.0]),
        ...
        traj.append(
            ...
            timestep_first(
                ...
                env_id=[1, 2],
                env_info={
                    "velocity@max": to_tensor([-1.0, neg_inf]),
        ...
        traj.append(
            timestep_last(
                ...
                env_id=[1, 2],
                env_info={
                    "velocity@max": to_tensor([0.0, neg_inf]),
                    "success": to_tensor([neg_inf, 1.0]),
        ...
        self.assertEqual(
            {  # Only two episodes are valid for velocity: -1 and 0
                ...
                "success": torch.as_tensor(2.0 / 3),
                # Two episodes are 2 and the third one is 5
                "value@step": torch.as_tensor(3.0),
            },
            metric.result(),
        )
