...
class FrameCropTest(parameterized.TestCase, alf.test.TestCase):
    ...
    @parameterized.parameters(
        (0, 0, 1, 1, "channels_last"),
        (0, 0, 1, 2, "channels_last"),
        (0, 0, 2, 1, "channels_last"),
        (1, 0, 1, 1, "channels_last"),
        (0, 1, 1, 1, "channels_last"),
        (0, 0, 2, 2, "channels_last"),
        (0, 0, 1, 1, "channels_first"),
        (0, 0, 1, 2, "channels_first"),
        (0, 0, 2, 1, "channels_first"),
        (1, 0, 1, 1, "channels_first"),
        (0, 1, 1, 1, "channels_first"),
        (0, 0, 2, 2, "channels_first"),
    )
    def test_frame_crop(self, sx, sy, width, height, channel_order):
        env = self._create_env(
            sx=sx, sy=sy, width=width, height=height, channel_order=channel_order
        )
        obs = env.reset()
        all_shapes = (
            obs["image"].shape,
...
class ActionWrappersTest(alf.test.TestCase):
    ...
    def test_continuous_action_mapping(self):
        ...
        env = ContinuousActionClip(
            ContinuousActionMapping(unwrapped, low=-1.0, high=2.0)
        )
        action = np.array([-1.1])
        env.step(action)
        ...
        action = np.array([2.1])
        env.step(action)
        self.assertTrue(np.all(unwrapped.action == unwrapped.action_space.high))

        action = np.array([0.0])  # from [-1, 2] to [-2, 3]
        env.step(action)
        self.assertAlmostEqual(float(unwrapped.action), -1.0 / 3)
