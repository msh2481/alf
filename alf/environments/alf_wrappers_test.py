...
class CurriculumWrapperTest(alf.test.TestCase):
    ...
    def test_curriculum_wrapper(self):
        ...
        for i in task_names:
            self.assertEqual(
                env.env_info_spec()["curriculum_task_count"][i], alf.TensorSpec(())
            )
            self.assertEqual(
                env.env_info_spec()["curriculum_task_score"][i], alf.TensorSpec(())
            )
            self.assertEqual(
                env.env_info_spec()["curriculum_task_prob"][i], alf.TensorSpec(())
            )
        ...
        for j in range(500):
            ...
            sum_probs = sum(time_step.env_info["curriculum_task_prob"].values())
            self.assertTrue(
                torch.all((sum_probs == 0.0) | ((sum_probs - 1.0).abs() < 1e-3))
            )
...
class DiscreteWrapperTest(parameterized.TestCase, alf.test.TestCase):
    ...
    def test_discrete_action_wrapper(self, actions_num, env_name, batched):
        ...
        self.assertTrue(
            np.allclose(
                actual_actions[1].cpu().numpy() - low,
