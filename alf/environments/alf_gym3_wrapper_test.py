...
class GymWrapperOnProcgenTest(alf.test.TestCase):
    ...
    def test_overriden_specs(self):
        ...
        self.assertEqual(
            BoundedTensorSpec(
                shape=(), dtype=torch.int32, minimum=np.array(0), maximum=np.array(14)
            ),
            self._env.action_spec(),
        )

    def test_step(self):
        action = zero_tensor_from_nested_spec(
        ...
        self.assertEqual(
            0,
            __observation_diff(
                time_step.observation["rgb"][0], new_time_step.observation["rgb"][0]
            ),
        )
        self.assertNotEqual(
            0,
            __observation_diff(
                time_step.observation["rgb"][1], new_time_step.observation["rgb"][1]
            ),
        )
        self.assertEqual(
            0,
            __observation_diff(
                time_step.observation["rgb"][2], new_time_step.observation["rgb"][2]
            ),
        )
        self.assertNotEqual(
            0,
            __observation_diff(
                time_step.observation["rgb"][3], new_time_step.observation["rgb"][3]
            ),
        )
