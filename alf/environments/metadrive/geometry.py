...
class FieldOfView(object):
    ...
    def __init__(self, front: float = 60.0, rear: float = 40.0, lateral: float = 30.0):
        ...
        """
        self._bbox = np.array(
            [[-rear, -lateral], [front, -lateral], [front, lateral], [-rear, lateral]]
        )
    ...
    def within(self, points: np.ndarray) -> np.ndarray:
        ...
        return np.all(
            np.logical_and(points >= self._bbox[0], points <= self._bbox[2]), axis=-1
        )
...
class Polyline(NamedTuple):
    ...
    def transformed_within_fov(
        ...
        return Polyline(
            point=transformed.point[within_bbox], category=self.category[within_bbox]
        )
