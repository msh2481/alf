...
def slow_env_load(
    observation_spec, action_spec, env_name, env_id=0, time_sleep=0, reset_sleep=0
):
...
class ParallelAlfEnvironmentTest(parameterized.TestCase, alf.test.TestCase):
    ...
    def _set_default_specs(self):
        ...
        self.action_spec = ts.BoundedTensorSpec(
            [7], dtype=torch.float32, minimum=-1.0, maximum=1.0
        )
        self.time_step_spec = ds.time_step_spec(
            self.observation_spec, self.action_spec, ts.TensorSpec(())
        )

    def _make_parallel_environment(
        self,
        constructor=None,
        num_envs=2,
        ...
        if batch_size_per_env > 1:
            constructor = functools.partial(
                _batch_env_ctor, constructor, batch_size_per_env
            )
    ...
    def test_non_blocking_reset_with_spare_envs(self):
        ...
        self.assertLessEqual(
            ...
            sleep_time - 0.1,
            msg=(
                f"Without spare env, Reset already called, "
    ...
    def test_non_blocking_start_processes_in_parallel(self):
        ...
        env = self._make_parallel_environment(
            constructor=constructor, num_envs=10, start_serially=False, blocking=False
        )
        end_time = time.time()
        self.assertLessEqual(
            end_time - start_time,
...
class FastParallelEnvironmentTest(ParallelAlfEnvironmentTest):
    ...
    def _check_same_nest(self, nest1, nest2):
        ...
        def _same_tensor(path, t1, t2):
            self.assertTrue(
                (t1 == t2).all(), f"different value at path '{path}': {t1} vs. {t2}"
            )
    ...
    @parameterized.parameters(
        (FastParallelEnvironment,), (parallel_environment.ParallelAlfEnvironment,)
    )
