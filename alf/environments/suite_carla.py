...
def adjust_weather_parameters(
    weather_param: carla.WeatherParameters, delta: WeatherParameters
):
    ...
    for m in delta.get_weather_fields():
        setattr(weather_param, m, getattr(weather_param, m) + getattr(delta, m))
...
class Player(object):
    ...
    def __init__(
        ...
        max_red_light_penalty=10.0,
        overspeed_penalty_weight=0.0,
        sparse_reward=False,
        sparse_reward_interval=10.0,
        ...
        self._observation_desc["velocity"] = (
            "3D Velocity relative to self coordinate in m/s"
        )
    ...
    def reset(self):
        ...
        self._goal_location = np.array(
            [goal_loc.x, goal_loc.y, goal_loc.z], dtype=np.float32
        )
        ...
        for v in self._alf_world.get_actors():
            if v.id == self._actor.id:
                continue
            forbidden_locations.append(self._alf_world.get_actor_location(v.id))
        ...
        while not ok and i < 100:
            ...
            for other_loc in forbidden_locations:
                if loc.distance(other_loc) < 10.0:
                    ok = False
                    break
            i += 1
        ...
        self._prev_action = np.zeros(self.action_spec().shape, dtype=np.float32)
        ...
        self._intermediate_goal_index = min(
            self._sparse_reward_index_interval, self._navigation.num_waypoints - 1
        )
        ...
        self._unrecorded_distance_reward = 0.0
    ...
    def action_spec(self):
        ...
        if self._controller is not None:
            ...
            return alf.BoundedTensorSpec(
                [4], minimum=[-1.0, -1.0, -1.0, 0.0], maximum=[1.0, 1.0, 1.0, 1.0]
            )
    ...
    def _get_goal(self):
        return _calculate_relative_position(
            self._actor.get_transform(), self._goal_location
        )
    ...
    def get_current_time_step(self, current_frame):
        ...
        self._alf_world.update_actor_location(
            self._actor.id, self._actor.get_location()
        )
        v = self._actor.get_velocity()
        ...
        if self._collision and not self._prev_collision:
            ...
            logging.info(
                "actor=%d frame=%d COLLISION" % (self._actor.id, current_frame)
            )
            ...
            reward -= min(
                ...
                Player.PENALTY_RATE_COLLISION * max(0.0, self._episode_reward),
            )
            reward_vector[Player.REWARD_COLLISION] = 1.0
        ...
        if (
            encountered_red_light_id is not None
            and encountered_red_light_id != self._prev_encountered_red_light_id
        ):
            logging.info(
                "actor=%d frame=%d Encountering RED_LIGHT"
        ...
        if (
            red_light_id is not None
            and red_light_id != self._prev_violated_red_light_id
        ):
            speed = self._get_agent_speed()
            logging.info(
                "actor=%d frame=%d Running RED_LIGHT speed %2.1f"
                % (self._actor.id, current_frame, speed)
            )
            reward_vector[Player.REWARD_RED_LIGHT] = 1.0
        ...
        if self._max_frame is None:
            ...
            self._current_distance < self._success_distance_thresh
            and self._actor.get_velocity() == carla.Location(0.0, 0.0, 0.0)
        ):
            ...
            self._terminate_upon_infraction == "redlight"
            or self._terminate_upon_infraction == "all"
        ) and reward_vector[Player.REWARD_RED_LIGHT] > 0:
            # directly terminate upon redlight infractions; the corresponding
            # infraction penalty has already been assigned earlier
            step_type = ds.StepType.LAST
            discount = 0.0
            logging.info(
                "actor=%d frame=%d FAILURE: red light infraction"
                % (self._actor.id, current_frame)
            )

        elif (
            self._terminate_upon_infraction == "collision"
            or self._terminate_upon_infraction == "all"
        ) and reward_vector[Player.REWARD_COLLISION] > 0:
            # directly terminate upon collision infractions; the corresponding
            # infraction penalty has already been assigned earlier
            step_type = ds.StepType.LAST
            discount = 0.0
            logging.info(
                "actor=%d frame=%d FAILURE: collision infraction"
                % (self._actor.id, current_frame)
            )

        elif (
            self._collision_loc is not None
            ...
            logging.info(
                "actor=%d frame=%d FAILURE: stuck at collision"
                % (self._actor.id, current_frame)
            )
            step_type = ds.StepType.LAST
        else:
        ...
        if self._sparse_reward:
            ...
            if step_type == ds.StepType.LAST and info["success"] == 1.0:
                ...
                distance_reward = np.linalg.norm(
                    self._intermediate_start - self._goal_location
                ...
                distance_reward = np.linalg.norm(
                    intermediate_goal - self._intermediate_start
                )
            ...
            distance_reward = np.linalg.norm(prev_loc - goal0) - np.linalg.norm(
                curr_loc - goal0
            )
        ...
        if overspeed > 0 and self._overspeed_penalty_weight > 0:
            logging.info(
                "actor=%d frame=%d OVERSPEED" % (self._actor.id, current_frame)
            )
    ...
    def act(self, action):
        ...
        if self._data_collection_mode:
            ...
            assert self._controller is None, (
                "controller is not supported " "in data collection currently"
            )
                ...
                if rgb_img is not None:
                    concat_img = np.zeros(
                        (
                            ...
                            rgb_img.shape[1] + bev_img.shape[1],
                            3,
                        ),
                        np.uint8,
                    )
    ...
    def _draw_ego_traj_on_image(
        ...
        nav_world = self._ego_to_world_position(
            ego_interp, append_self=append_self, forward_shift_delta=forward_shift_delta
        )
        if zero_world_z:
            nav_world[:, 2] = 0
    ...
    def _ego_to_world_position(
        self, ego_points, append_self=False, forward_shift_delta=0
    ):
        """
        Args:
            ego_points (np.ndarray): [N, 3]
    ...
    def __init__(
        ...
        if use_docker:
            ...
            self._process = subprocess.Popen(
                command.split(), stdout=sys.stdout, stderr=sys.stderr, env=new_env
            )
...
class CarlaEnvironment(AlfEnvironment):
    ...
    def _spawn_vehicles(self):
        ...
        assert len(blueprints) > 0, (
            "Cannot find safe vehicle '%s'" % self._vehicle_filter
        )
    ...
    def _update_weather(self):
        ...
        if self._weather_length_count == 0:
            ...
            self._actual_weather_length = (
                max(0.25, np.random.rand()) * self._max_weather_length
            )
    ...
    def _spawn_walkers(self):
        ...
        for response, walker_speed, spawn_point in zip(
            ...
            if response.error:
                logging.error("%s: %s" % (response.error, spawn_point.location))
                continue
        ...
        for walker in self._walkers:
            commands.append(
                carla.command.SpawnActor(
                    walker_controller_bp, carla.Transform(), walker["walker"].id
                )
            )
        responses = self._client.apply_batch_sync(commands, True)
        ...
        for walker, walker_speed in zip(self._walkers, walker_speeds):
            ...
            self._alf_world.update_actor_location(walker["walker"].id, location)
    ...
    def _update_time_of_the_day(self):
        light_state = None
        if (
            0.25 * self._day_length - self._step_time
            < self._time_of_the_day
            <= 0.25 * self._day_length
        ):
            light_state = carla.VehicleLightState.NONE
        elif (
            0.75 * self._day_length - self._step_time
            < self._time_of_the_day
