...
class TimeLimit(AlfEnvironmentBaseWrapper):
    ...
    def __init__(self, env, duration):
        ...
        assert (
            self.batch_size is None or self.batch_size == 1
...
class ActionObservationWrapper(AlfEnvironmentBaseWrapper):
    ...
    def _add_action(self, time_step):
        return time_step._replace(
            observation=dict(
                observation=time_step.observation, prev_action=time_step.prev_action
            )
        )
...
class ScalarRewardWrapper(AlfEnvironmentBaseWrapper):
    ...
    def __init__(self, env, reward_weights=None):
        ...
        if reward_weights is None:
            reward_weights = [1.0] + [0.0] * (rewards_n - 1)
        assert (
            isinstance(reward_weights, (list, tuple))
...
class CurriculumWrapper(AlfEnvironmentBaseWrapper):
    ...
    def __init__(
        ...
        assert (
            len(env.action_spec()) == 2
            and "action" in env.action_spec()
    ...
    def _update_curriculum(self, task_ids, task_scores, task_episode_lengths):
        for task_id, task_score in zip(task_ids, task_scores):
            ...
            self._current_episode_lengths[
                task_ids
            ] += self._current_score_update_rate * (
                task_episode_lengths - self._current_episode_lengths[task_ids]
            )
            self._task_counts[task_id] += 1
            self._current_scores[task_id] += self._current_score_update_rate * (
    ...
    def _to_batched_tensor(raw):
        ...
        return nest.map_structure(
            lambda x: (
                ...
                if isinstance(x, (np.ndarray, np.number, float, int))
                else x
            ),
            raw,
        )
...
class TensorWrapper(AlfEnvironmentBaseWrapper):
    ...
    def __init__(self, env):
        assert env.batched, "TensorWrapper can only be used to wrap batched env"
        super().__init__(env)
...
class TemporallyCorrelatedNoiseWrapper(AlfEnvironmentBaseWrapper):
    ...
    def _step(self, action):
        ...
        noisy_action = (
            action + self._past_noise_weight * self._past_noise + current_noise
        )
        ...
        noisy_action = np.clip(
            ...
            a_max=self._action_spec.maximum,
        ).astype(np.float32)
...
class NormalizedActionWrapper(AlfEnvironmentBaseWrapper):
    ...
    def __init__(self, env: AlfEnvironment):
        ...
        self._action_spec = nest.map_structure(
            lambda spec: alf.BoundedTensorSpec(
                minimum=-1.0, maximum=1.0, shape=spec.shape, dtype=spec.dtype
            ),
            action_spec,
        )
        self._time_step_spec = env.time_step_spec()._replace(
    ...
    def _step(self, action):
        ...
        scaled_action = nest.map_structure_up_to(
            action, _scale_back, action, self._affine_paras
        )
...
class BatchEnvironmentWrapper(AlfEnvironment):
    ...
    def _step(self, action):
        ...
        time_step = alf.nest.map_structure(
