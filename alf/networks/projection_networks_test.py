...
class TestNormalProjectionNetwork(parameterized.TestCase, alf.test.TestCase):
    ...
    def test_squash_mean_normal_projection_net(self, network_ctor):
        ...
        self.assertRaises(AssertionError, network_ctor, input_spec, action_spec)

        action_spec = BoundedTensorSpec(
            (2,), torch.float32, minimum=(0, -0.01), maximum=(0.01, 0)
        )
        net = network_ctor(
    ...
    def test_parallel_normal_projection_network(
        self, network_ctor, state_dependent_std
    ):
    ...
    def test_beta_projection_net(self):
        ...
        embedding = torch.rand((100,) + input_spec.shape, dtype=torch.float32)
        action_spec = BoundedTensorSpec(
            (8,), minimum=-1.0, maximum=1.0, dtype=torch.float32
        )

        net = BetaProjectionNetwork(
            input_spec.shape[0], action_spec, projection_output_init_gain=0.0
        )
    ...
    def test_parallel_beta_projection_net(self):
        ...
        embedding = torch.rand((100,) + input_spec.shape, dtype=torch.float32)
        action_spec = BoundedTensorSpec(
            (2,), minimum=-1.0, maximum=1.0, dtype=torch.float32
        )
    ...
    def test_straight_through_gradient(self, mode):
        ...
        if mode == "plain":
            ...
            self.assertTensorNotClose(
                p_layer.weight.grad, torch.zeros(p_layer.weight.grad.shape)
            )
            self.assertTensorNotClose(
                p_layer.bias.grad, torch.zeros(p_layer.bias.grad.shape)
            )
