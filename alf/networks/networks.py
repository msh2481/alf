...
class LSTMCell(Network):
    ...
    def __init__(self, input_size, hidden_size, name="LSTMCell"):
        ...
        """
        state_spec = (alf.TensorSpec((hidden_size,)), alf.TensorSpec((hidden_size,)))
        super().__init__(
...
class Residue(Network):
    ...
    def __init__(
        self, block, input_tensor_spec=None, activation=torch.relu_, name="Residue"
    ):
        """
        Args:
            block (Callable):
...
class NoisyFC(Network):
    ...
    def __init__(
        ...
        super().__init__(
            input_tensor_spec=alf.TensorSpec((input_size,)),
            state_spec=(alf.TensorSpec((input_size,)), alf.TensorSpec((output_size,))),
        ),
