...
class NestworksTest(alf.test.TestCase):
    ...
    def test_temporal_pool_max(self):
        ...
        for i in range(5, 8):
            o, state = l(x[:, i, :], state)
            self.assertEqual(
                o[:, 3:, :], x[:, 0:6, :].reshape(batch_size, 2, 3, dim).max(dim=2)[0]
            )
            self.assertEqual(o[:, :3, :], torch.zeros((batch_size, 3, dim)))
        ...
        for i in range(8, 11):
            o, state = l(x[:, i, :], state)
            self.assertEqual(
                o[:, 2:, :], x[:, 0:9, :].reshape(batch_size, 3, 3, dim).max(dim=2)[0]
            )
            self.assertEqual(o[:, :2:], torch.zeros((batch_size, 2, dim)))
        ...
        for i in range(11, 14):
            o, state = l(x[:, i, :], state)
            self.assertEqual(
                o[:, 1:, :], x[:, 0:12, :].reshape(batch_size, 4, 3, dim).max(dim=2)[0]
            )
            self.assertEqual(o[:, :1:], torch.zeros((batch_size, 1, dim)))
    ...
    def test_temporal_pool_avg(self):
        ...
        for i in range(5, 8):
            ...
            self.assertTensorClose(
                o[:, 3:, :], x[:, 0:6, :].reshape(batch_size, 2, 3, dim).mean(dim=2)
            )
            self.assertEqual(o[:, :3, :], torch.zeros((batch_size, 3, dim)))
        ...
        for i in range(8, 11):
            o, state = l(x[:, i, :], state)
            self.assertTensorClose(
                o[:, 2:, :], x[:, 0:9, :].reshape(batch_size, 3, 3, dim).mean(dim=2)
            )
            self.assertEqual(o[:, :2:], torch.zeros((batch_size, 2, dim)))
        ...
        for i in range(11, 14):
            o, state = l(x[:, i, :], state)
            self.assertTensorClose(
                o[:, 1:, :], x[:, 0:12, :].reshape(batch_size, 4, 3, dim).mean(dim=2)
            )
            self.assertEqual(o[:, :1:], torch.zeros((batch_size, 1, dim)))
    ...
    def test_noisy_net(self):
        ...
        self.assertEqual(
            l.state_spec, (alf.TensorSpec((dim0,)), alf.TensorSpec((dim1,)))
        )
        batch_size = 10000
