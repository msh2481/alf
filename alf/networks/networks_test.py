...
class NestworksTest(alf.test.TestCase):
    ...
    def test_temporal_pool_max(self):
        ...
        for i in range(5, 8):
            ...
            self.assertEqual(
                o[:, 3:, :],
                x[:, 0:6, :].reshape(batch_size, 2, 3, dim).max(dim=2)[0],
            )
            self.assertEqual(o[:, :3, :], torch.zeros((batch_size, 3, dim)))
        ...
        for i in range(8, 11):
            ...
            self.assertEqual(
                o[:, 2:, :],
                x[:, 0:9, :].reshape(batch_size, 3, 3, dim).max(dim=2)[0],
            )
            self.assertEqual(o[:, :2:], torch.zeros((batch_size, 2, dim)))
        ...
        for i in range(11, 14):
            ...
            self.assertEqual(
                o[:, 1:, :],
                x[:, 0:12, :].reshape(batch_size, 4, 3, dim).max(dim=2)[0],
            )
            self.assertEqual(o[:, :1:], torch.zeros((batch_size, 1, dim)))
    ...
    def test_temporal_pool_avg(self):
        ...
        for i in range(5, 8):
            ...
            self.assertTensorClose(
                o[:, 3:, :],
                x[:, 0:6, :].reshape(batch_size, 2, 3, dim).mean(dim=2),
            )
            self.assertEqual(o[:, :3, :], torch.zeros((batch_size, 3, dim)))
        ...
        for i in range(8, 11):
            ...
            self.assertTensorClose(
                o[:, 2:, :],
                x[:, 0:9, :].reshape(batch_size, 3, 3, dim).mean(dim=2),
            )
            self.assertEqual(o[:, :2:], torch.zeros((batch_size, 2, dim)))
        ...
        for i in range(11, 14):
            ...
            self.assertTensorClose(
                o[:, 1:, :],
                x[:, 0:12, :].reshape(batch_size, 4, 3, dim).mean(dim=2),
            )
            self.assertEqual(o[:, :1:], torch.zeros((batch_size, 1, dim)))
