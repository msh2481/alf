...
def py_flatten_up_to(shallow_nest, nest):
    ...
    if isinstance(shallow_nest, list) or is_unnamedtuple(shallow_nest):
        ...
        for fv1, fv2 in zip(
            extract_fields_from_nest(shallow_nest), extract_fields_from_nest(nest)
        ):
    ...
    def _traverse_transform(nests, levels):
        ...
        if is_namedtuple(nests[0]):
            new_vals = _traverse_transform(
                nests=[getattr(nest, level) for nest in nests], levels=levels[1:]
            )
            return [
...
def transpose(nested: Nest, shallow_nest: Nest = None, new_shallow_nest: Nest = None):
    ...
    new_nest = map_structure_up_to(
        new_shallow_nest, lambda flat: pack_sequence_as(shallow_nest, flat), new_nest
    )
    return new_nest
