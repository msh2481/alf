...
class TestFlattenUpTo(parameterized.TestCase, alf.test.TestCase):
    ...
    @parameterized.parameters(
        (nest.py_flatten_up_to, AssertionError),
...
class TestAssertSameStructure(parameterized.TestCase, alf.test.TestCase):
    ...
    @parameterized.parameters(
        (nest.py_assert_same_structure, AssertionError),
    ...
    def test_assert_same_structure(self, assert_same_structure, error):
        ...
        nest1 = NTuple(
            a=1, b=NTuple(a=NTuple(a=(2,), b=[3]), b=dict(x=NTuple(a=[100], b=1)))
        )
        nest2 = NTuple(
            b=NTuple(a=NTuple(a=(2,), b=[300]), b=dict(x=NTuple(a=[1], b=100))), a=3.0
        )
        ...
        self.assertRaises(
            error, assert_same_structure, NTuple(a=1, b=[2]), NTuple(b=[2], a=[1])
        )
...
class TestMapStructure(parameterized.TestCase, alf.test.TestCase):
    ...
    @parameterized.parameters(
        (nest.py_map_structure, AssertionError), (cnest.map_structure, RuntimeError)
    )
...
class TestFastMapStructure(alf.test.TestCase):
    ...
    def test_fast_map_structure(self):
        ...
        self.assertEqual(
            nest.fast_map_structure(lambda a, b: a + b, [1, 3], [4, 5]), [5, 8]
        )
...
class TestMapStructureUpTo(parameterized.TestCase, alf.test.TestCase):
    ...
    @parameterized.parameters(
        (nest.py_map_structure_up_to, AssertionError),
    ...
    def test_different_lengths(self, map_structure_up_to, error):
        self.assertRaises(
            error, map_structure_up_to, [1, 2, 3], lambda x: x * 2, [1, 2]
        )
        self.assertRaises(
            error, map_structure_up_to, [1, 2], lambda x, y: x * y, [1, 2, 3], [4, 5]
        )
        self.assertRaises(
            error,
            map_structure_up_to,
...
class TestMapStructureUpToWithPath(alf.test.TestCase):
    ...
    def test_basic_functionality(self):
        ...
        self.assertEqual(
            called, [("0", "v0", "u0"), ("1.0", "v1", "u1"), ("1.1", "v2", "u2")]
        )
    ...
    def test_structure_mismatch_raises(self):
        ...
        with self.assertRaises(AssertionError):
            nest.py_map_structure_up_to_with_path(
                shallow_nest, dummy_func, deep1, deep2
            )
...
class TestPackSequenceAs(parameterized.TestCase, alf.test.TestCase):
    ...
    def test_pack_sequence_as_order(self):
        self.assertEqual(
            nest.py_pack_sequence_as({"b": 1, "a": 2}, [3, 4]),
...
class TestNestConcat(alf.test.TestCase):
    ...
    def test_nest_concat_tensors(self):
        ntuple = NTuple(
            a=dict(x=torch.zeros((2, 3)), y=torch.zeros((2, 4))), b=torch.zeros((2, 10))
        )
    ...
    def test_nest_concat_specs(self):
        ntuple = NTuple(
            a=dict(x=TensorSpec((2, 3)), y=TensorSpec((2, 4))), b=TensorSpec((2, 10))
        )
        ret = NestConcat()(ntuple)
...
class TestNestSum(alf.test.TestCase):
    ...
    def test_nest_sum_tensors(self):
        ntuple = NTuple(
            a=dict(x=torch.zeros(()), y=torch.zeros((2, 4))), b=torch.zeros((4,))
        )
    ...
    def test_nest_sum_specs(self):
        ntuple = NTuple(
            a=dict(x=TensorSpec(()), y=TensorSpec((2, 4))), b=TensorSpec((4,))
        )
        ret = NestSum()(ntuple)  # broadcasting
...
class TestNestMultiply(alf.test.TestCase):
    ...
    def test_nest_multiply_tensors(self):
        ntuple = NTuple(
            a=dict(x=torch.zeros(()), y=torch.ones((2, 4))), b=torch.ones((4,))
        )
...
class TestNestOuterProduct(parameterized.TestCase, alf.test.TestCase):
    ...
    def test_nest_outer_product(self, padding):
        ...
        tensors = [torch.tensor([[1, 2], [1, 2]]), torch.tensor([[3, 4], [3, 4]])]
        ret = NestOuterProduct(batch_dims=1, padding=padding)(tensors)
        if padding:
            self.assertTensorEqual(
                ret,
                torch.tensor(
                    [[3, 4, 1, 6, 8, 2, 3, 4, 1], [3, 4, 1, 6, 8, 2, 3, 4, 1]]
                ),
            )
        else:
    ...
    def test_nest_outer_product_specs(self, padding):
        ...
        self.assertEqual(
            ret, TensorSpec(((1 + padding) * (2 * 4 + padding) * (4 + padding),))
        )
...
class TestPruneNestLike(parameterized.TestCase, alf.test.TestCase):
    ...
    @parameterized.parameters(
        (nest.py_prune_nest_like, ValueError),
...
class TestTransformNests(alf.test.TestCase):
    ...
    def test_transform_nests(self):
        ...
        ntuple_b = NTuple(
            a=dict(x=torch.ones(()), y=torch.ones((2, 4))), b=torch.ones((4,))
        )
...
class TestTransposeNest(alf.test.TestCase):
    ...
    def test_transpose_nest(self):
        ...
        self.assertEqual(
            transposed_nest2,
            dict(x=NTuple(a=3, b=5), y=dict(n=NTuple(a=1, b=1), m=NTuple(a=2, b=3))),
        )
