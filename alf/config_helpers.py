...
def adjust_config_by_multi_process_divider(
    ddp_rank: int, multi_process_divider: int = 1
):
    ...
    # information.
    if ddp_rank > 0:
        config1("TrainerConfig.evaluate", False, raise_if_used=False, override_all=True)
    ...
    try:
        if conf_params:
            for conf_param in conf_params:
                ...
                if pos == -1:
                    raise ValueError(
                        "conf_param should have a format of "
...
def get_env():
    ...
    if _env is None:
        ...
        adjust_config_by_multi_process_divider(
            PerProcessContext().ddp_rank, PerProcessContext().num_processes
        )
