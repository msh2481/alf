...
observation_preprocessors = {
    "image": torch.nn.Sequential(
        alf.layers.Permute(2, 0, 1),
        alf.layers.Cast(),
        alf.layers.Conv2D(3, encoding_dim, kernel_size=3),
        alf.layers.Conv2D(encoding_dim, encoding_dim, kernel_size=3),
        alf.layers.Reshape((encoding_dim, -1)),
        alf.layers.Transpose(),
    ),
    "direction": torch.nn.Sequential(
        torch.nn.Embedding(4, encoding_dim), alf.layers.Reshape((1, encoding_dim))
    ),
    "mission": torch.nn.Sequential(
...
input_preprocessors = {
    ...
    "prev_action": torch.nn.Sequential(
        torch.nn.Embedding(7, encoding_dim), alf.layers.Reshape((1, encoding_dim))
    ),
}
...
value_network_ctor = partial(
    alf.networks.ValueNetwork, fc_layer_params=fc_layers_params, activation=activation
)
...
alf.config(
    "TracAlgorithm", ac_algorithm_cls=PPOAlgorithm, action_dist_clip_per_dim=0.01
)
