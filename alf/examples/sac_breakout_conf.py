...
CONV_LAYER_PARAMS = define_config(
    "CONV_LAYER_PARAMS", ac_breakout_conf.CONV_LAYER_PARAMS
)

q_network_cls = functools.partial(
    QNetwork, fc_layer_params=FC_LAYER_PARAMS, conv_layer_params=CONV_LAYER_PARAMS
)
...
alf.config(
    ...
    mini_batch_length=2,
    num_updates_per_train_iter=4,  # 8GB GPU memory: 40 updates, 250x2 timesteps, 84x84, 3 convs, 4 frames stacked
    mini_batch_size=500,
    unroll_length=8,
    ...
    replay_buffer_length=33334,
)  # 20GB CPU memory: 30 envs, 84x84
