...
class MoNetVisualizer(Algorithm):
    ...
    def _tensor_to_image(self, tensor, height=None, width=None):
        ...
        img = (torch.clamp(tensor, 0.0, 1.0) * 255).to(torch.uint8)
        if img.ndim == 3:
        ...
        if render.is_rendering_enabled():
            ...
            def _overlay_slot_selection(slot_sel, height):
                slot_sel = self._tensor_to_image(
                    torch.cat(
                        (slot_sel, torch.zeros((2,) + slot_sel.shape[1:3])), dim=0
                    ),
                    height=height,
                )
            ...
            selection_maps = [
                        ...
                        render.Image.stack_images(
                            [
                                _overlay_slot_selection(
                                    slot_selection[:, i, ...], height=256  # [1,h,w]
                                )
                            ]
                        ),
                    ],
                ...
                for i in range(slot_selection.shape[1])
            ]
...
class MoNetAgent(RLAlgorithm):
    ...
    def predict_step(self, time_step, state):
        ...
        vi_step = self._vis.predict_step((alg_step.info, time_step.observation))
...
alf.config(
    "suite_simple.load",
    env_args=dict(N=64, noise_level=0.0, color=True),
...
alf.config(
    ...
    decoder_cls=partial(
        ...
        conv_layer_params=((32, 3, 1),) * 4 + ((obs_spec.shape[0] + 1, 3, 1),),
        fc_layer_params=(32,),
    ),
