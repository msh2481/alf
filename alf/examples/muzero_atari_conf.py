...
scale_grad_by_num_quantiles = define_config(
    "scale_grad_by_num_quantiles", isinstance(rv_loss, losses.QuantileRegressionLoss)
)
...
if suite == "ATARI":
    ...
    alf.config(
        "MuzeroRepresentationImpl", reanalyze_batch_size=1280 if use_small_net else 640
    )
elif suite == "PROCGEN":
...
lr_schedule = define_config(
    ...
    StepScheduler(
        "percent",
        [(0.45, initial_lr), (0.9, 0.1 * initial_lr), (1.0, 0.01 * initial_lr)],
        warm_up_period=0.01,
...
def create_representation_net(observation_spec):
    ...
    return alf.nn.Sequential(
        alf.layers.Scale(1.0 / 255.0),
        alf.layers.Conv2D(in_channels, 32, kernel_size=3, strides=2, padding=1),
...
def create_dynamics_net(input_tensor_spec):
    ...
    num_actions = action_spec.maximum + 1.0
    return alf.nn.Sequential(
        lambda x: torch.cat(
            [
                x[0],
        ...
        a=alf.layers.Conv2D(
            num_planes + 1, num_planes, 3, padding=1, activation=alf.math.identity
        ),
        ...
        if lstm:
            if use_small_net:
                return [
                    ...
                    alf.layers.FC(
                        ...
                        weight_opt_args=dict(fixed_norm=pred_fixed_weight_norm),
                        use_bn=pred_use_bn,
                    ),
                ]
            else:
            ...
            if use_small_net:
                return [
                    alf.layers.Reshape(-1),
                    alf.layers.FC(
                        latent_dim, 1024, activation=activation, use_bn=use_bn
                    ),
                    alf.layers.FC(
                        1024,
                        dim,
    ...
    action_net = alf.layers.Sequential(
        ...
        *_make_trunk(),
        alf.nn.CategoricalProjectionNetwork(
            ...
            bias_opt_args=dict(
                weight_decay=action_bias_decay, l2_regularization=action_bias_l2
            ),
        ),
    )
    game_over_net = (
        alf.layers.Sequential(
            *_make_trunk(),
            alf.layers.FC(
                dim,
                1,
...
alf.config(
    ...
    reanalyze_td_steps_func=LinearTdStepFunc(  # LinearMaxAgeTdStepFunc(),
