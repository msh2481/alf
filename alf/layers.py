...
def normalize_along_batch_dims(x, mean, variance, variance_epsilon):
    ...
    assert spec == TensorSpec.from_tensor(
        variance
...
class FixedDecodingLayer(nn.Module):
    ...
    def __init__(self, input_size, output_size, basis_type="rbf", sigma=1.0, tau=0.5):
        ...
        assert basis_type in {
            "poly",
            "cheb",
            "rbf",
            "haar",
        }, "the specified method " "{} is not supported".format(basis_type)
        ...
        def _polyvander_matrix(n, D, tau=tau):
            ...
            B = torch.as_tensor(np.polynomial.polynomial.polyvander(x.cpu(), D))
        ...
        def _haar_matrix(n, tau=tau):
            ...
            def _get_haar_matrix(n):
                ...
                h_i = torch.sqrt(torch.tensor([n / 2.0])) * _kron(
                    torch.eye(len(h)), torch.tensor([[1], [-1]], dtype=torch.float32)
                )
                # combine both parts
...
class FC(nn.Module):
    ...
    def __init__(
        ...
        method: Literal["linear", "fused_linear_act", "default"] = "default",
        ...
        if method == "fused_linear_act" and not use_bn and not use_ln:
            if activation in (F.relu_, F.relu, torch.relu, torch.relu_) or isinstance(
                activation, nn.ReLU
            ):
                self._act_name = "RELU"
...
class ParallelFC(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if inputs.ndim == 2:
            assert (
                inputs.shape[1] == l
            ), "inputs has wrong shape %s. Expecting (B, %d)" % (inputs.shape, l)
            inputs = inputs.unsqueeze(0).expand(n, *inputs.shape)
        elif inputs.ndim == 3:
            assert (
                inputs.shape[1] == n and inputs.shape[2] == l
            ), "inputs has wrong shape %s. Expecting (B, %d, %d)" % (inputs.shape, n, l)
        ...
        if self._output_size == 1:
            ...
            y = torch.baddbmm(
                self._bias.unsqueeze(1), inputs, self.weight.transpose(1, 2)
            )  # [n, B, k]
        else:
...
class Conv2DBatchEnsemble(Conv2D):
    ...
    def forward(self, inputs):
        ...
        s = (
            self._s[ensemble_ids].unsqueeze_(-1).unsqueeze_(-1)
        )  # [B, out_channels, 1, 1]
...
class ParallelConv2D(nn.Module):
    ...
    def forward(self, img):
        ...
        if img.ndim == 4:
            ...
            assert (
                img.shape[1] == self._in_channels
            ), "Input img has wrong shape %s. Expecting (B, %d, H, W)" % (
                img.shape,
                self._in_channels,
            )

            img = img.unsqueeze(1).expand(img.shape[0], self._n, *img.shape[1:])
        elif img.ndim == 5:
            # the non-shared case
            assert (
...
class ParallelConvTranspose2D(nn.Module):
    ...
    def forward(self, img):
        ...
        # reshape back: [B, n*C', H', W'] -> [B, n, C', H', W']
        res = res.reshape(
            res.shape[0], self._n, self._out_channels, res.shape[2], res.shape[3]
        )
        return self._activation(res)
...
class ParamFC(nn.Module):
    ...
    def set_parameters(self, theta, reinitialize=False):
        ...
        assert (
            theta.ndim == 2 and theta.shape[1] == self.param_length
...
class ParamConv2D(nn.Module):
    ...
    def _set_weight(self, weight, reinitialize=False):
        ...
        if weight.shape[0] == 1:
            # non-parallel weight
            weight = weight.view(
                self._out_channels, self._in_channels, self._kH, self._kW
            )
        else:
            # parallel weight
            weight = weight.view(
            ...
            weight = weight.reshape(
                self._n_groups * self._out_channels,
                self._in_channels,
                self._kH,
                self._kW,
            )
    ...
    def forward(self, img, keep_group_dim=True):
        ...
        if self._n_groups == 1:
            # non-parallel layer
            assert (
                img.ndim == 4 and img.shape[1] == self._in_channels
            ), "Input img has wrong shape %s. Expecting (B, %d, H, W)" % (
                img.shape,
                self._in_channels,
            )
...
@alf.configurable(
    whitelist=["with_batch_normalization", "bn_ctor", "weight_opt_args", "activation"]
)
class ResidueBlock(nn.Module):
    ...
    def __init__(
        ...
        conv1 = conv_fn(
            in_channels, channels, kernel_size, stride, padding=padding, bias=bias
        )
...
class TransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self._attention_scale = d_k**-0.5 if scale_attention_score else 1.0
...
class RandomCrop(nn.Module):
    ...
    def __init__(
        self, size: Union[int, Tuple[int]], padding: Union[int, Tuple[int]] = 0
    ):
        super().__init__()
        if type(size) == int:
            size = (size, size)
...
class NaiveParallelLayer(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self._n > 1:
            ...
            output = alf.nest.map_structure(
                lambda tensor: tensor.unsqueeze(1), outputs[0]
            )
    ...
    def _make_spec(spec):
        if type(spec) == alf.TensorSpec:
            ...
            return alf.BoundedTensorSpec(
                (n,) + spec.shape, spec.dtype, spec.minimum, spec.maximum
            )
