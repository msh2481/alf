...
def create_sac_and_inputs():
    ...
    sac = SacAlgorithm(
        ...
        actor_network_cls=partial(
            alf.networks.ActorDistributionNetwork, fc_layer_params=(1024,) * 5
        ),
...
class TensorRTUtilsTest(parameterized.TestCase, alf.test.TestCase):
    ...
    def test_onnxruntime_backends(self):
        ...
        if torch.cuda.is_available():
            self.assertTrue(
                "CUDAExecutionProvider" in providers, "Need to install onnxruntime-gpu!"
            )
            expected_providers.insert(0, "CUDAExecutionProvider")
        if is_tensorrt_available():
            self.assertTrue(
                "TensorrtExecutionProvider" in providers, "tensorrt installation error!"
            )
    ...
    def test_compile_method(self, tensorrt_backend):
        ...
        if not tensorrt_backend:
            ...
            os.environ["ORT_ONNX_BACKEND_EXCLUDE_PROVIDERS"] = (
    ...
    def test_tensorrt_engine(self):
        ...
        print(f"TensorRT predict step time: ", (time.time() - start_time) / 100)
    ...
    def test_tensorrt_engine_cache(self):
        ...
        compile_method(
            ...
            "predict_step",
            get_tensorrt_engine_class(
                validate_args=True, engine_file=engine_file, force_build_engine=True
            ),
        )
        start_time = time.time()
        alg.predict_step(timestep, state=state)  # build engine
    ...
    def test_tensorrt_resnet50(self):
        ...
        if is_tensorrt_available():
            for fp16 in [True, False]:
                ...
                print(
                    f"TensorRT FP{fp_str} predict step time: ",
        ...
        if is_onnxruntime_available():
            ...
            for _ in range(100):
                output = model1(dummy_img)
            print(f"ONNX-runtime predict step time: ", (time.time() - start_time) / 100)
