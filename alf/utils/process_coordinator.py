...
class Coordinator(object):
        ...
        with self._lock:
            ...
            if not self._stop_event.is_set():
                if ex and self._exc_info_to_raise is None:
                    if isinstance(ex, tuple):
                        logging.info(
                            "Error reported to Coordinator: %s", str(ex[1]), exc_info=ex
                        )
                        self._exc_info_to_raise = ex
                    else:
                        logging.info(
                            "Error reported to Coordinator: %s, %s", type(ex), str(ex)
                        )
    ...
    def join(
        self, processes=None, stop_grace_period_secs=120, ignore_live_processes=False
    ):
