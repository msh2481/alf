...
class BipartiteMatchingLossTest(parameterized.TestCase, alf.test.TestCase):
    ...
    def test_forward_loss(self):
        ...
        target = torch.tensor(
            [[[0.9, 0.9, 0.9], [0, 0, 0.1]], [[0.1, 0.1, 0.1], [0.5, 0.6, 0.5]]]
        )
    ...
    def test_loss_training(self):
        ...
        model = torch.nn.Sequential(
            ...
            alf.layers.FC(d_model, d_model, torch.relu_),
            *transform_layers,
        ...
        for _ in range(epochs):
            ...
            for i in range(0, idx.shape[0], batch_size):
                ...
                b_pred = b_pred[:, 1:, :]
                cost_mat = torch.cdist(b_pred, b_target, p=2.0)
        ...
        val_pred = val_pred[:, 1:, :]
        cost_mat = torch.cdist(val_pred, val_target, p=2.0)
    ...
    def test_loss_training_discrete_target(self):
        ...
        optimizer = torch.optim.Adam(
            list(model.parameters()) + list(input_fc.parameters()) + [queries], lr=1e-3
        )
        epochs = 5
        batch_size = 100
        matcher = losses.BipartiteMatchingLoss(reduction="mean")
