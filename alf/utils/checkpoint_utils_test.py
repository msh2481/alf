...
class ComposedAlg(Algorithm):
    ...
    def __init__(
        self, optimizer=None, sub_alg1=None, sub_alg2=None, params=[], name="SimpleAlg"
    ):
        super().__init__(optimizer=optimizer, name=name)
...
class TestWithParamSharing(alf.test.TestCase):
    ...
    def test_with_param_sharing(self):
        with tempfile.TemporaryDirectory() as ckpt_dir:
            ...
            self.assertTrue(
                (
                    alg_root._sub_alg2.state_dict()["ignored_param"]
                    == torch.tensor([-1.0])
                )
            )
            self.assertTrue(
                (
                    alg_root.state_dict()["_sub_alg1._param_list.0"]
                    == torch.tensor([-1.0])
                )
            )
        ...
        with tempfile.TemporaryDirectory() as ckpt_dir:
            ...
            expected_state_dict = OrderedDict(
                [
                    ...
                    ("_sub_alg2._param_list.0", torch.tensor([2.0])),
                    (
                    ...
                    ("_param_list.0", torch.tensor([0.0])),
                    (
                        "_optimizers.0",
                        {
                            "state": {},
    ...
    def test_parallel_network_state_dict_and_params(self, lstm):
        ...
        if lstm:
            network_ctor = functools.partial(
                LSTMEncodingNetwork, hidden_size=(1,), post_fc_layer_params=(2, 2)
            )
        else:
        ...
        network_w_preprocessor = network_ctor(
            input_tensor_spec=input_spec, input_preprocessors=input_preprocessors
        )
        ...
        for network in [network_wo_preprocessor, network_w_preprocessor]:
            ...
            n_net = alf.networks.network.NaiveParallelNetwork(network, replicas)
