    ...
    def __init__(
        ...
        with alf.device(self._device):
            self.register_buffer(
                "_current_size", torch.zeros(num_environments, dtype=torch.int64)
            )
            ...
            self.register_buffer(
                "_current_pos", torch.zeros(num_environments, dtype=torch.int64)
            )
    ...
    def _enqueue(self, batch, env_ids=None):
        ...
        with alf.device(self._device):
            ...
            if (
                ...
                self._current_size.fill_(min(current_pos + 1, self._max_length))
            else:
...
class DataBuffer(RingBuffer):
    ...
    def __init__(
        self, data_spec: TensorSpec, capacity, device="cpu", name="DataBuffer"
    ):
        """
        Args:
    ...
    def get_batch(self, batch_size):
        ...
        with alf.device(self._device):
            indices = torch.randint(
                low=0, high=self.current_size, size=(batch_size,), dtype=torch.int64
            )
