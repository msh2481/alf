...
class Checkpointer(object):
    ...
    def load(
        ...
        def _load_one(module, checkpoint):
            ...
            if not including_replay_buffer:
                missing_keys = list(
                    filter(lambda k: not k.startswith("_replay_buffer."), missing_keys)
                )
            if strict:
                error_msgs = []
    ...
    def save(self, global_step, suffix: Optional[str] = None):
        ...
        if self._global_step == -1:
            ...
            with open(
                os.path.join(self._ckpt_dir, "ckpt-structure-optimizer.json"), "w"
            ) as outfile:
                json.dump(_use_placeholder_value(optimizer_state), outfile, indent=4)
            with open(
                os.path.join(self._ckpt_dir, "ckpt-structure-replay_buffer.json"), "w"
            ) as outfile:
                json.dump(
                    _use_placeholder_value(replay_buffer_state), outfile, indent=4
                )
