...
class _DiscreteRegressionLossBase(ScalarPredictionLoss):
    ...
    def __init__(
        self, transform: Optional[InvertibleTransform] = None, inverse_after_mean=False
    ):
        super().__init__()
        self._transform = transform
        if self._transform is not None:
    ...
    def forward(self, pred: Tensor, target: Tensor) -> Tensor:
        ...
        if self._debug_summaries and alf.summary.should_record_summaries():
            ...
            summary_utils.add_mean_hist_summary(
                "predicted_projected_pred_norm", predicted_projected_pred.norm(dim=1)
            )
...
class MeanSquaredLoss(object):
    ...
    def __init__(
        self, batch_dims: int = 1, debug_summaries: bool = True, name: str = "MSELoss"
    ):
        super().__init__()
        self._debug_summaries = debug_summaries
        self._name = name
