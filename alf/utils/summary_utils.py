...
def summarize_variables(name_and_params, with_histogram=True):
    ...
    for var_name, var in name_and_params:
        ...
        alf.summary.scalar(
            name="summarize_vars/" + var_name + "_value_norm", data=var_values.norm()
        )
    ...
    for var_name, var in name_and_params:
        ...
        if with_histogram:
            if torch.all(grad_values.isfinite()):
                alf.summary.histogram(
                    name="summarize_grads/" + var_name + "_gradient", data=grad_values
                )
        alf.summary.scalar(
...
def summarize_per_category_loss(
    ...
    if loss_info.batch_label != ():
        ...
        for label in labels:
            ...
            alf.summary.scalar(
                "loss/loss_for_category_{}".format(label_str), data=subset_loss.mean()
            )
            if summarize_count:
                alf.summary.scalar(
    ...
    * Tensor: each dimension dist[..., a] will be summarized
    ...
    def _summarize_one(path, dist):
        if isinstance(dist, torch.Tensor):
            ...
            if isinstance(
                dist,
                (td.Normal, dist_utils.StableCauchy, dist_utils.TruncatedDistribution),
            ):
                loc = dist.loc
