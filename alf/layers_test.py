...
class LayersTest(parameterized.TestCase, alf.test.TestCase):
    ...
    def test_parallel_fc(
        ...
        fc = alf.layers.FC(
            x_dim, 6, activation=act, use_bias=use_bias, use_bn=use_bn, use_ln=use_ln
        )
    ...
    @parameterized.parameters(
        ...
        dict(
            n=5, act=torch.relu, use_bias=True, use_bn=True, specify_comp_weight=False
        ),
        dict(
            n=5, act=torch.relu, use_bias=True, use_ln=True, specify_comp_weight=False
        ),
    )
    ...
    def test_compositional_fc_chaining(
        ...
        cfc2 = alf.layers.CompositionalFC(
            8, 6, n=n, activation=act, use_bias=use_bias, use_bn=use_bn, use_ln=use_ln
        )
    ...
    def test_parallel_conv(
        self, n=2, act=math_ops.identity, use_bias=True, use_bn=False, parallel_x=True
    ):
        batch_size = 5
        in_channels = 4
        out_channels = 3
    ...
    def test_param_fc(
        self, batch_size=1, n=2, act=math_ops.identity, use_bias=True, use_ln=False
    ):
        input_size = 4
        output_size = 5
        ...
        fc = alf.layers.FC(
            input_size, output_size, activation=act, use_ln=use_ln, use_bias=use_bias
        )
        ...
        if use_ln:
            self.assertEqual(
                pfc._ln.param_length, fc._ln.weight.nelement() + fc._ln.bias.nelement()
            )
    ...
    def test_fixed_decoding_layer(self, basis_type, input_size, output_size, sigma):
        ...
        if (
            ...
            dec_no_basis_weighting = alf.layers.FixedDecodingLayer(
                input_size, output_size, basis_type=basis_type, sigma=sigma, tau=1.0
            )
        ...
        if input_size <= 8:
            ...
            if input_size == 2:
                ...
                expected_haar_basis = torch.as_tensor(
                    ...
                    / 2
                    * np.array(
                        [[1, 1, 1, 1], [1, 1, -1, -1], [st, -st, 0, 0], [0, 0, st, -st]]
                    ).transpose(1, 0)
                )
            elif input_size == 8:
                # H8^T
    ...
    def test_transformer_block(self, task_type=2):
        ...
        for i in range(iters):
            ...
            logging.log_every_n(
                logging.INFO, "%s loss=%s" % (i, loss.detach().cpu().numpy()), n=100
            )
            loss.backward()
            optimizer.step()
    ...
    def test_causal_conv1d_shape(
        self, kernel_size, dilation, batch_size=5, act=math_ops.identity, use_bias=True
    ):
        in_channels = 4
        out_channels = 5
        signal_length = 40
    ...
    def test_conv2d(self):
        ...
        self._test_make_parallel(
            layer, input_spec, get_pnet_parameters=lambda pnet: (pnet.weight, pnet.bias)
        )
