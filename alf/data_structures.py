...
def _generate_time_step(
    ...
    if all(map(_is_numpy_array, flat_observation)):
        ...
        assert all(
            map(torch.is_tensor, flat_observation)
        ), "Elements in observation must be Tensor"
        md = torch
        if reward is not None:
    ...
    return TimeStep(
        step_type, reward, discount, observation, prev_action, env_id, env_info=env_info
    )
...
BasicRLInfo = elastic_namedtuple(
    "BasicRLInfo",
    [
        "action",
    ],
