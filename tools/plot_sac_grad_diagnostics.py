...
def metric_plot_df(
    ...
    *,
    ...
    seed_curve = df.group_by(["experiment", "seed", x_col],
                             maintain_order=True).agg(
                                 pl.col(y_col).mean().alias(y_col)).sort(
                                     ["experiment", "seed", x_col])
    summary = bootstrap_ci(seed_curve,
                           group_cols=("experiment", ),
                           x_col=x_col,
                           value_col=y_col,
...
def _plot_iqm_panel(ax,
                    ...
                    *,
                    ...
                    symlog_linthresh: float = 1.0) -> None:
    if summary.is_empty() or x_col not in summary.columns:
        _hide_ax(ax, title, f"No IQM data for `{y_label}`")
        return
    ...
    groups = sorted(summary["experiment"].unique().to_list())
    for i, group in enumerate(groups):
        color = colors[i % len(colors)]
        s = summary.filter(pl.col("experiment") == group).sort(x_col)
        if s.is_empty():
            continue
        x = s[x_col].to_numpy()
        y = s["stat"].to_numpy()
        ...
        ax.plot(x, y, color=color, linewidth=2.0, label=group)
...
def plot_dashboard(by_type: dict[str, pl.DataFrame], *, out: str) -> None:
    ...
    _plot_iqm_panel(axes[0],
                    ep_summary,
                    x_col="episode_idx",
                    title=f"Episode Return IQM ({int(CONFIDENCE * 100)}% CI)",
                    y_label="episode_return")
    _plot_iqm_panel(
        axes[1],
        align_summary,
        x_col="train_iter",
        title=f"Grad Alignment Cosine IQM ({int(CONFIDENCE * 100)}% CI)",
        y_label="grad_align_cos_mean")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot SAC-Grad alignment cosine diagnostics.")
